...
import os
# from test import visualize_graph
...
def load_ddi_data_fold(name, fold, batch_size, data_size_ratio, valid_ratio=0.2):
    ...
    if dataset_name in ('twosides', 'drugbank'):
        ...
        #
        # for graph in all_drug_data.values():
        #     graph_data = graph
        ...
        # train_positive_tuple = train_tup[0]
        # t_p_t_data = pd.DataFrame(train_positive_tuple)
        # t_p_t_data.to_csv('t_p_t_data.csv')

        # val_positive_tuple = val_tup[0]
        # v_p_t_data = pd.DataFrame(val_positive_tuple)
        # v_p_t_data.to_csv('v_p_t_data.csv')

        # train_negative_tuple = train_tup[1]
        # t_n_t_data = pd.DataFrame(train_negative_tuple)
        # t_n_t_data.to_csv('t_n_t_data.csv')

        # val_negative_tuple = val_tup[1]
        # v_n_t_data = pd.DataFrame(val_negative_tuple)
        # v_n_t_data.to_csv('v_n_t_data.csv')

        test_tup = load_split(f'test_fold{fold}')

        # test_positive_tuple = test_tup[0]
        # test_p_t_data = pd.DataFrame(test_positive_tuple)
        # test_p_t_data.to_csv('test_p_t_data.csv')





        # test_negative_tuple = test_tup[1]
        # test_n_t_data = pd.DataFrame(test_negative_tuple)
        # test_n_t_data.to_csv('test_n_t_data.csv')
...
class DrugDataset(Dataset):
    ...
    def collate_fn(self, batch):
        ...
        combo_indices_pos = [] #原有的组合的索引
        combo_indices_neg = [] #新的组合的索引
        combo_indices_neg2 = []
        already_in_combo = {}
        rels = []
        batch_unique_pairs= []
...
#这里往上全部是旧的
            combo_indices_neg2.append(combo_idx)
            rels.append(int(r))
        ...
        batch_drug_pair_indices = torch.LongTensor(combo_indices_pos + combo_indices_neg + combo_indices_neg2) #已有的和新有的的组合的索引的组合
...
#总的图数据（旧的+新的），【旧的+新的组合的j_num_indices, i_num_indices(这俩是原来的组合的头部药和尾部药的原子数量，有多少个原子就有多少个0）,
    # pair_edge_index】，r, 见上，头部和尾部的所有的点的索引(旧+新组合）0开始
    # *两组两组存入的，例如如果头部数据没有变，j_num_indices就是重复了两遍同样的
    ...
    def _get_new_batch_id_and_num_nodes(self, old_id, old_id_to_new_batch_id, batch_drug_feats):
        ...
        return new_id, num_nodes #new_id:把id变成了序号
#combo是旧的换了索引
    ...
    def _get_combo_index(self, combo, old_combo, already_in_combo, unique_pairs, num_nodes):
        ...
        if idx == -1:
            ...
            if pair_edge_index is None:
                index_j = torch.arange(num_nodes[0]).repeat_interleave(num_nodes[1]) #一个一个元素重复后面遍
                index_i = torch.arange(num_nodes[1]).repeat(num_nodes[0]) #整个重复后面遍
                pair_edge_index = torch.stack([index_j, index_i])  #构建出了一个连接矩阵 ；2行756列；其实就是两个药原子的两两组合，一个27，一个28， （0，1）就是第一个药的第一个原子和
                # 第二个药的第二个原子
                bipartite_edge_dict[old_combo] = pair_edge_index

            j_num_indices, i_num_indices = drug_num_node_indices[old_combo[0]], drug_num_node_indices[old_combo[1]] #drugnumnodeindices:有几个原子就有几个0
